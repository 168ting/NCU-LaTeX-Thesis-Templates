...
\def\deptshort {永續與綠能科技研究學院}
\def\dept      {永續與綠能科技碩士學位學程}%XX學系XXX碩士班(請參考「中央大學學位論文撰寫體例參考」附錄)
\def\degree    {碩士} % 碩士/博士
\def\titleZh   {一種基於 Line Bot 的智慧家電控制系統}   % Chinese title
\def\titleEn   {A Smart Home Control Systems Base on the Line Bot} % English title
...
\def\profs     {江士元}
\def\degreedate{中~華~民~國~一~一~五~年~六~月}
\def\copyyear  {2025-2026} % 版權頁上「年份範圍」，不會自動出現，要搭配\printcopyright 相關區塊使用。

\setboolean{printcopyright}{false} % {true}/{false} print copyright text on titlepage or cover.

\def\keywordsZh{關鍵字, 論文, 樣板}
\def\keywordsEn{Keyword, Thesis, Template}
...
\def\titlepageFontFamily{\rmfamily\setmainfont{Times New Roman}\CJKfamily{sf}}
...
\setboolean{reallyBlankPage}{true} % {true}/{false} true for use  really blank pages between chapters. 
